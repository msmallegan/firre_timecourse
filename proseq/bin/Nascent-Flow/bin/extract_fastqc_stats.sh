...
GC=$(unzip -c "$(find . -maxdepth 1 -name "*R1_fastqc.zip")" "$SRR"_R1_fastqc/fastqc_data.txt \
         | grep "%GC" | grep -o "[0-9]*")
SEQ=$(unzip -c "$(find . -maxdepth 1 -name "*R1_fastqc.zip")" "$SRR"_R1_fastqc/fastqc_data.txt | \
          grep "Total Sequences" | \
          grep -o "[0-9]*")
DEDUP=$(unzip -c "$(find . -maxdepth 1 -name "*R1_fastqc.zip")" "$SRR"_R1_fastqc/fastqc_data.txt | \
