---
title: "Delta Differential Expression: Firre KO mESC dox"
...
res_tbl_list <- mapply(
    ...
    MoreArgs = list(
        ...
        headerLevel = 2,
        write = TRUE,
        dir = paste0(params$results_dir,"/ko_dox_shrunken_deltas", collapse = "")
    ),
    SIMPLIFY = FALSE,
    USE.NAMES = TRUE
)
...
```{r}
fl <- list.files("results/ko_dox_shrunken_deltas/", full.names = T)
fl <- fl[grep("all", fl)]
...
write.csv(dat, "ko_dox_shrunken_deltas.csv", row.names = FALSE)
```
...
g + geom_bar(stat = "identity", position = "dodge") + 
  ggtitle("Dox mESC KO | delta DEG count")
ggsave("dox_mesc_ko_delta_deg_count.pdf")
```
