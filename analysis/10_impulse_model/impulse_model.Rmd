...
# One gene
esc_rlog_countsl <- esc_rlog_countsl %>%
  arrange(-abs(rlog_count))

# Remove sample with a lot of outlier genes
# JR2432
esc_rlog_countsl <- esc_rlog_countsl %>% filter(sample_id != "JR2432")
...
esc_rlog_counts_zero_centered <- esc_rlog_countsl %>%
  ...
  summarize(abundance = mean(rlog_count)) %>%
  arrange(-abs(abundance))

# glist <- c("Firre", "Sox2", "Nanog", "Adgrg1", "Duox1", "Gfap")
glist <- c("Id3")
glist <- c("ENSMUSG00000116207.1")
start_time <- Sys.time()
...
one_gene$condition
one_gene$sigmoid_fit[[1]]
plot_timecourse_fits("Id3", one_gene)
