...
geo_rnaseq_samples <- samples %>%
  ...
  mutate(replicate_numeric = replicate, 
         ...
         treatment = "dox") %>%
  unite(geo_sample_name, experiment, firre_induced, timepoint, treatment, replicate, remove = FALSE) %>%
...
geo_samples <- bind_rows(geo_rnaseq_samples %>%
  dplyr::select(geo_sample_name, fastq1, fastq2, geo_fastq1, geo_fastq2, type), 
  geo_atacseq_samples %>%
  dplyr::select(geo_sample_name, fastq1, fastq2, geo_fastq1, geo_fastq2, type))

gs_fastq1 <- geo_samples %>% 
  dplyr::select(geo_sample_name, fastq1, geo_fastq1, type) %>%
  dplyr::rename(fiji_fastq = fastq1, geo_fastq = geo_fastq1)
gs_fastq2 <- geo_samples %>% 
  dplyr::select(geo_sample_name, fastq2, geo_fastq2, type) %>%
  dplyr::rename(fiji_fastq = fastq2, geo_fastq = geo_fastq2)

gs_mv_script <- bind_rows(gs_fastq1, gs_fastq2) %>%
  rowwise() %>%
  mutate(fiji_location = ifelse(type == "RNAseq", paste0("/scratch/Shares/rinn/Michael/firre_timecourse/rnaseq/fastq/", fiji_fastq),
                                paste0("/scratch/Shares/rinn/Michael/firre_timecourse/atacseq/fastq/", fiji_fastq)),
         geo_location = paste0("/scratch/Shares/rinn/Michael/firre_timecourse/geo_upload/", geo_fastq),
         mv_script = paste0("mv ", fiji_location, " ", geo_location))

writeLines(c("#!/bin/bash", gs_mv_script$mv_script), "geo_staging_raw_files.sh")
...
md5_df <- data.frame(md5sum = md5_list) %>%
  separate(md5sum, into = c("md5sum", "filename"), sep = "  ../geo_upload/")
write_csv(md5_df, "geo_md5sum.csv")


# Let's get some processed data files ready for the GEO submission. 
rnaseq_counts <- read.csv("../../rnaseq/results/salmon/salmon_merged_gene_counts.csv") %>%
  merge(g2s) %>%
  dplyr::select(gene_id, gene_name, everything()) %>%
  filter(gene_id %in% genes$gene_id) %>%
