...
```{r import-samplesheets}
# Check raw data file locations
chipseq_samples <- read_csv("../../chipseq/chipseq_samplesheet.csv",
                            col_types = cols(
                              .default = col_character(),
                              replicate = col_double(),
                              experiment_replicate = col_double()
                            ))
rnaseq_samples <- read_csv("../../rnaseq/rnaseq_samplesheet.csv",
                           col_types = cols(
                             sample_id = col_character(),
                             cell_type = col_character(),
                             firre_ko = col_character(),
                             firre_induced = col_character(),
                             timepoint = col_double(),
                             replicate = col_double(),
                             experiment_replicate = col_double(),
                             timecourse_length = col_character(),
                             date_sequenced = col_character(),
                             sample_name = col_character(),
                             raw_data_read1 = col_character(),
                             raw_data_read2 = col_character(),
                             raw_data_read1_file_name = col_character(),
                             raw_data_read2_file_name = col_character(),
                             md5_read1 = col_character(),
                             md5_read2 = col_character()
                           ))
...
proseq_samples <- read_csv("../../proseq/proseq_samplesheet.csv",
                           col_types = cols(
                             ...
                             experiment_replicate = col_double(),
                             timecourse_length = col_character(),
                             date_sequenced = col_date(format = ""),
                             sample_name = col_character(),
                             geo_sample_name = col_character(),
                             geo_sample_read1 = col_character(),
                             geo_sample_read2 = col_character(),
                             raw_data_read1 = col_character(),
                             raw_data_read2 = col_character(),
                             raw_data_read1_file_name = col_character(),
                             raw_data_read2_file_name = col_character(),
...
assert(all(file.exists(chipseq_samples$raw_data_read1, chipseq_samples$raw_data_read2)))
assert(all(file.exists(rnaseq_samples$raw_data_read1, rnaseq_samples$raw_data_read2)))
assert(all(file.exists(atacseq_samples$raw_data_read1, atacseq_samples$raw_data_read2)))
assert(all(file.exists(proseq_samples$raw_data_read1, proseq_samples$raw_data_read2)))
```
...
if(!file.exists("../util/gencode.vM25.annotation.genes.gtf")) {
  download.file("https://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_mouse/release_M25/gencode.vM25.annotation.gtf.gz",
                "../util/gencode.vM25.annotation.gtf.gz")
  gencode <- rtracklayer::import(file.path("../util",
...
tx2gene <- read.csv("/scratch/Shares/rinn/Michael/firre_timecourse/rnaseq/results/salmon/tx2gene.csv")
```

```{r rnaseq-expression-data}
### COUNTS
tpm <- read.csv("/scratch/Shares/rinn/Michael/firre_timecourse/rnaseq/results/salmon/salmon_merged_gene_tpm.csv") %>%
  merge(g2s) %>%
...
salmon_gene_counts <- read_rds("data/salmon_gene_counts.rds")
...
assert(names(genes) == rownames(salmon_gene_counts))
...
rownames(rnaseq_samples) <- rnaseq_samples$sample_id

assert(all(colnames(salmon_gene_counts) %in% rnaseq_samples$sample_id))

counts <- salmon_gene_counts[,as.character(rnaseq_samples$sample_id)]
assert(all(rownames(rnaseq_samples) == colnames(counts)))
assert(all(rownames(salmon_gene_counts) == genes$gene_id))

nonzero_genes <- counts[rowSums(counts) > 0,]
num_measured_genes <- nrow(nonzero_genes)

# Save as a bundle of objects.
save(genes, g2s, tx2gene, tpm, rnaseq_samples, counts, num_measured_genes,
