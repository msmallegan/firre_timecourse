...
### COUNTS
tpm <- read_rds("data/salmon_merged_gene_tpm.rds")
...
assert(all(colnames(tpm)[grep("JR", colnames(tpm))] %in% rnaseq_samples$sample_id))

samples <- rnaseq_samples %>%
  filter(sample_id %in% colnames(salmon_gene_counts))
counts <- salmon_gene_counts[,as.character(samples$sample_id)]
assert(all(rownames(samples) == colnames(counts)))
...
save(genes, g2s, tx2gene, tpm, rnaseq_samples, samples, counts, num_measured_genes,
