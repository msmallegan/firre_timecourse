...
```{r}
andergassen <- read_csv("data/andergassen_2019_deg.csv")
```
...
datf <- dat %>% filter(padj < 0.05, abs(log2FoldChange) >= log2(1.5))
datf <- datf[which(!grepl("wt_firre", datf$contrast)), ]
datf <- datf[which(!grepl("ko_firre", datf$contrast)), ]

length(unique(datf$gene_id))
unique(datf$contrast)
...
gsum <- dat_sum %>%
  group_by(gene_id, gene_name, tissue) %>%
  distinct() %>%
  summarize(count = n()) %>%
  arrange(-count)
...
# Add Firre entries
firre_gcount <- tibble(count = 7:15,
                       num_genes = 1,
                       cumgenes = 1)
...
# scale_y_log10(expand = c(0, 0), limits = c(1,10000)) + scale_x_continuous(breaks = 1:6, limits = c(0,7), expand = c(0, 0)) +
...
ggsave("figures/previous_firre_ko_mouse_deg.pdf", height = 3, width = 4)
...
# 
# 775 / length(unique(dat_sum$gene_id))
# 
# 
# gsum2 <- dat_sum %>% 
#   rowwise() %>%
#   mutate(tissue = unlist(strsplit(unlist(strsplit(contrast, "/"))[[2]], "_"))[[1]]) %>%
#   dplyr::select(gene_id, gene_name, direction, tissue) %>%
#   distinct() %>%
#   group_by(gene_id, gene_name) %>%
#   summarize(count = n())
#   table(gsum2$count)
# # group_by(gene_id, gene_name, contrast)
# # source("_setup.R")
# 
# gsum2 <- dat_sum %>% 
#   rowwise() %>%
