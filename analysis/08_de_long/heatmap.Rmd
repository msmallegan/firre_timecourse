...
annot <- annot %>% 
  arrange(firre_ko, firre_induced, timepoint)
...
annot <- annot %>% select(timepoint, firre_induced, firre_ko)
# let's make a heatmap
htmap <- pheatmap(tpm_norm, cluster_cols = FALSE, show_rownames = FALSE, 
         ...
         annotation_colors = ann_colors)
pdf("ftc_heatmap.pdf", width = 12, height = 9)
grid::grid.newpage()
...
custom.config$local_connectivity <- 0.5
custom.config$bandwidth <- 1.5
custom.config$alpha <- 1.5
custom.config$gamma <- 1.5
custom.config$spread <- 2
custom.config$set_op_mix_ratio <- 3
# help(umap.defaults)
tpm_umap <- umap(tpm_norm, custom.config)
...
g <- ggplot(umap_layout, aes(x = x, y = y, color = frdiff))
g + geom_point()
