...
unbiased_regions <- counts_stranded %>%
  filter(abs_strand_bias < 1) %>%
  left_join(bidir_coords) %>%
  mutate(width = end - start)

hmm <- unbiased_regions %>%
  filter(pos_count > 100,
         neg_count > 100,
         width < 1000)

bidir_counts_filtered <- counts_stranded %>% filter(pos_count > 5, neg_count > 5, abs_strand_bias < 1) %>%
  left_join(bidir_coords) %>%
  mutate(width = end - start)

ggdensity(counts_stranded %>% filter(pos_count > 5, neg_count > 5), x = "strand_bias")

# Bidir regions to include
bidir_filtered <- unique(bidir_counts_filtered$bidir_id)
```
...
de_atac_counts_scaled <- t(scale(t(de_atac_counts)))
pheatmap::pheatmap(de_atac_counts_scaled, cluster_cols = F)
...
So this result is that there are no significantly changing bidirectionals...
Before we go back to trying DESeq2 or something, let's just calculate the lm of the CPM per bidir.
Another thing to consider is starting from the coverage filtered bidirectionals
so that the number of statistical tests performed is smaller.
...
ggplot(res_vs_zero %>% filter(bidir_id %in% sig_lrt_res$bidir_id,
                              abs(log2FoldChange) > log2(2)), aes(x = log2FoldChange, y = -log10(padj))) +
  geom_point() +
  facet_wrap(~comparison)
