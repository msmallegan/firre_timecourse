...
bidir_counts_filtered <- counts_stranded %>% 
  filter(pos_count >= 5, neg_count >= 5)
```
...
bidir_counts_sbfiltered <- counts_stranded %>% 
  filter(pos_count >= 5, neg_count >= 5, abs_strand_bias <= 1)
```
...
sig_res_vs_zero <- res_vs_zero %>%
  filter(bidir_id %in% sig_lrt_res$bidir_id)
...
Are the bidirs that continue their trajectory more likely to be those associated with ATAC-seq peaks?
Is there a correlation with read counts? In other words, may this be an artifact of the DESeq2 dispersion normalization?

```{r}
ggplot(res_vs_zero %>% filter(comparison == "timepoint_15_vs_0_shrnk"), aes(x = log10(baseMean), y = log2FoldChange)) +
  geom_point()

ggplot(res_vs_zero %>% filter(comparison == "timepoint_30_vs_0_shrnk"), aes(x = log10(baseMean), y = log2FoldChange)) +
  geom_point()
```
