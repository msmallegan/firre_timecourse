...
```{r}
fl <- list.files("results/2019-10-16/wt_firre_shrunken", full.names = T)
fl <- fl[grep("all", fl)]
...
write.csv(dat %>% select(contrast,baseMean,log2FoldChange,lfcSE,stat,pvalue,padj,geneID,gene_id,gene_name), "wt_firre_shrunken.csv", row.names = FALSE)
unique(dat$contrast)
unique(hmm$contrast)
hmm <- read.csv("timecourse_shrunken_lfc_results.csv")
# okay, let's remove the samples that are not shrunken.
hmm <- hmm[!grepl("wt_firre", hmm$contrast),]

# now let's add it back in.
hdat <- dat %>% select(contrast,baseMean,log2FoldChange,lfcSE,stat,pvalue,padj,geneID,gene_id,gene_name)

hmm <- bind_rows(hmm, hdat)
warnings()
write.csv(hmm, "timecourse_shrunken_lfc_results.csv", row.names = F)
