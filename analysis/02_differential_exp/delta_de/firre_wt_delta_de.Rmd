---
title: "Delta Differential Expression: Firre WT mESC Firre"
...
res_tbl_list <- mapply(
    ...
    MoreArgs = list(
        ...
        headerLevel = 2,
        write = TRUE,
        dir = paste0(params$results_dir,"/wt_firre_shrunken_deltas", collapse = "")
    ),
    SIMPLIFY = FALSE,
    USE.NAMES = TRUE
)
...
```{r}
fl <- list.files("results/wt_firre_shrunken_deltas/", full.names = T)
fl <- fl[grep("all", fl)]
...
write.csv(dat, "wt_firre_shrunken_deltas.csv", row.names = FALSE)
```
...
g + geom_bar(stat = "identity", position = "dodge") + 
  ggtitle("Firre mESC WT | delta DEG count")
ggsave("firre_mesc_wt_delta_deg_count.pdf")
```
