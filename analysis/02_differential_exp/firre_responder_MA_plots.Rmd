...
tpzero <- maplotdf %>% group_by(geneName) %>% 
  ...
  mutate(log2FoldChange = 0, lfcSE = 0, stat = 1, pvalue = 1, padj = 1, rankScore = NA,
         rank = NA, timepoint = 0) %>%
  select("baseMean", "log2FoldChange", "lfcSE", "stat", "pvalue", "padj", "rankScore", "rank", "isDE", "geneName", "timepoint")
...
maplotdf[which(abs(maplotdf$log2FoldChange) > 3), "label"] <- TRUE
...
g <- g +
  geom_text_repel(data = label_subset, nudge_x = 0.1, nudge_y = 0.1, family = "Helvetica",
...
firre_rescue_render_files <- animate(g,  
                                     ...
                                     start_pause = 20, 
                                     rewind = FALSE,
        renderer = file_renderer(dir = "figures/", 
