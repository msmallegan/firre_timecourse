...
# Fix the base mean problem here?
# TODO: Come up with a better threshold based on what DEseq2 originally set
# as the threshold for independent filtering. 
...
sig_dyn_res_esc <- dyn_res %>% filter(padj < 0.05,
                                      lfcSE < 1)
g <- ggplot(sig_dyn_res_esc, aes(x = timepoint, y = log2FoldChange, group = gene_name))
...
g <- ggplot(sig_dyn_res_esc %>% filter(gene_name != "Firre"), 
...
tpm_goi <- tpm %>% filter(gene_name == goi) %>%
  pivot_longer(3:ncol(.), names_to = "sample_id",
               values_to = "tpm")
tpm_npc_goi <- tpm_goi %>% filter(sample_id %in% npc_samples$sample_id) %>%
  merge(npc_samples)
tpm_esc_goi <- tpm_goi %>% filter(sample_id %in% esc_samples$sample_id) %>%
  merge(esc_samples)

tpm_goi <- bind_rows(tpm_esc_goi, tpm_npc_goi)
tpm_goi <- tpm_goi %>% 
