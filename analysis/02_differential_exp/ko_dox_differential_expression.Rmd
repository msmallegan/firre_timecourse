---
title: "Differential Expression: Dox timecourse vs. zero timepoint"
...
# Filter to just the non-inducible mESC knockout samples.
 filter(cell_type == "mESCs", 
        ...
        firre_expression_induced_by_dox == FALSE) %>%
...
res_tbl_list <- mapply(
    ...
    MoreArgs = list(
        ...
        headerLevel = 2,
        write = TRUE,
        dir = paste0(params$results_dir,"/ko_dox_shrunken", collapse = "")
    ),
    SIMPLIFY = FALSE,
    USE.NAMES = TRUE
)
...
# ordering by hand
deg_profiles$gene_name <- factor(deg_profiles$gene_name, levels = c(
  "Banp",  
  "Gm12070",
   "Robo4",
   "Spry4",
...
# sorting?
sorted <- deg_profiles %>% arrange(-earliest_timepoint,
                                   -num_timepoints_sig)
...
g <- ggplot(deg_profiles, aes(x = timepoint, y = gene_name, alpha = sig_alpha)) + 
  geom_tile(aes(fill = log2FoldChange)) + 
...
ggsave(filename = "dox_responsive_heatmap.pdf", height = 10, width = 7.5)


write.csv(deg_profiles, "results/dox_responders_firre_ko_mesc.csv", row.names = F)
```
