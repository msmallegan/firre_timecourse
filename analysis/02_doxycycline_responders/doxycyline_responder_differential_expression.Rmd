...
sig_res_shrunken <- res_shrunkendf %>%
  filter(padj < 0.05) %>%
...
g + geom_hline(yintercept = 0) +
  ...
  theme_paperwhite()
  # geom_text_repel(data = res_shrunkendf %>% filter(gene_name %in% genes_overlapping$gene_name, timepoint == 210 | timepoint == 270),
  #                 aes(label = gene_name))
