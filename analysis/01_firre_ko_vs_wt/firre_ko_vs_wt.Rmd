...
ltc_res <- results(ltc_dds, 
  ...
  rownames_to_column("gene_id") %>% 
  merge(g2s) %>%
  mutate(result_name = dynamic_res[1])

for(i in 2:length(dynamic_res)) {
  tmp_ltc_res <- results(ltc_dds, 
                   name = dynamic_res[i]) %>%
  as.data.frame() %>%
  rownames_to_column("gene_id") %>% 
  merge(g2s) %>%
  mutate(result_name = dynamic_res[i])
  ltc_res <- bind_rows(ltc_res, tmp_ltc_res)
}
...
ltc_sig_res <- ltc_res %>% filter(padj < 0.05)

max_fc <- ltc_sig_res %>%
  group_by(gene_id) %>%
  summarize(max_fc = max(abs(log2FoldChange))) 

ltc_sig_res <- ltc_sig_res %>%
  left_join(max_fc) 
