...
ko_rescue_dds <- DESeqDataSetFromMatrix(countData = ko_rescue_counts, 
                                        colData = ko_rescue_samples, 
                                        design = ~ firre_induced + timepoint + timecourse_length + timepoint*firre_induced)
ko_rescue_dds <- DESeq(ko_rescue_dds, test = "LRT", reduced = ~ firre_induced + timepoint + timecourse_length)
...
```{r}
esc_ko_tpm <- tpm %>%
  filter(gene_id %in% ko_rescue_sig$gene_id) %>%
  pivot_longer(3:ncol(.), names_to = "sample_id", values_to = "tpm") %>%
  left_join(samples) %>%
  filter(cell_type == "ESC", firre_ko == "KO", firre_induced == "firre_induced") %>%
  group_by(gene_id, timepoint) %>%
  summarize(tpm = mean(tpm)) %>%
  pivot_wider(names_from = "timepoint", values_from = "tpm", names_sort = TRUE) %>%
  column_to_rownames("gene_id") %>%
  as.matrix()
# expressed <- rowMax(esc_ko_tpm)
# esc_ko_tpm <- esc_ko_tpm[expressed > 1,]
...
esc_ko_scaled <- t(scale(t(esc_ko_tpm)))
pheatmap::pheatmap(esc_ko_scaled, cluster_cols = FALSE, show_rownames = FALSE)

# esc_ko_tpm_log10
table(is.infinite(esc_ko_tpm_log10))
esc_ko_tpm_log10 <- log10(esc_ko_tpm + 0.1)
var_rows <- apply(esc_ko_tpm_log10, 1, var)
summary(var_rows)
esc_ko_tpm_log10_var <- esc_ko_tpm_log10[var_rows > 0.09,]
# esc_ko_tpm_log10 <- esc_ko_tpm_log10 [rowSums(is.na(esc_ko_tpm_log10 )) , ]
pca_dat <- prcomp(t(esc_ko_tpm))
proportion_of_variance <- summary(pca_dat)$importance[2,1:2]
...
esc_ko_tpm_log10 <- log10(esc_ko_tpm)
esc_ko_var <- apply(esc_ko_tpm_log10, 1, var, na.rm = TRUE)

var_esc_ko <- esc_ko_tpm_log10[which(esc_ko_var > 0.02),]
var_esc_ko_scaled <- t(scale(t(var_esc_ko)))
pheatmap::pheatmap(var_esc_ko_scaled, cluster_cols = FALSE, show_rownames = FALSE)

summary(esc_ko_var)
hmm <- esc_ko_tpm[is.na(esc_ko_var),]

length(which(esc_ko_var))
hist(esc_ko_var)
```
